%% 姿态解算及KF滤波
%   前提：先有零速检测判断  调用ZeroDetection_V2.mlapp 计算
%   选项：
%       1.   利用惯导解算计算姿态，静止时进行速度置0
%       2.   利用KF滤波进行姿态估计
%       3.   所有静止状态的加速度计的数据，统计在一起，进行水平姿态求取并绘制  
 ...
 if Choice == 3
    ...
    % 坐标系  北东地  前右下     原始数据是右前上
        % 右前上 z轴输出1   前右下 z轴输出-1
    ...
    % 静止累积时长
        T_static = 100;
        Att(1:T_static,1) = IMU(1:T_static,1);
        
    % 水平姿态计算
        for i = T_static:L
            if (State_AG(i-T_static+1,2) == 1) && (State_AG(i,2) == 1)
                % 计算 T_static 采样周期内的重力矢量(是加速度计输出的相反)
                tp_Acc = -mean(IMU(i-T_static+1:i,2:4));
                tp_Mag = [1,0,0];      
                tp_q = ecompass(tp_Acc,tp_Mag);
                tp_att = euler(tp_q, 'ZYX', 'frame');      % 姿态角   航向 俯仰 横滚  单位：弧度
                Att(i,1) = IMU(i,1);
                Att(i,2) = tp_att(3);
                Att(i,3) = tp_att(2);
                Att(i,4) = tp_att(1);
...
%         % 初始 姿态      计算起始姿态 10s 加计数据
%         tp_Acc = -mean(IMU(1:1*Hz,2:4));
%         tp_Mag = [1,0,0]; 
%         tp_q = ecompass(tp_Acc,tp_Mag);
%         tp_att = euler(tp_q, 'ZYX', 'frame'); 
%         G_Start_Att(1,1) = tp_att(3)+deg2rad(0.5);   %起始姿态 欧拉角  x横滚 y俯仰 z航向  前右下
%         G_Start_Att(2,1) = tp_att(2)+deg2rad(0.5);   %姿态 俯仰角 度
%         G_Start_Att(3,1) = deg2rad(10)+deg2rad(1);   %姿态 航向角        
%     
% end
...
%         Result_AVP(1,8:10) = G_Start_Pos';       % 位置  经纬度高程
%         Result_AVP(1,11:13) = zeros(3,1);          %   离起始点 的 北东地距离
%         
%         Att_result_KF_Acc = zeros(L,4);                         
%         Att_result_KF_Acc(:,1) = IMU(1:L,1);                    
%         Att_result_KF_Acc(1,2:4) = INSData_Now.att;         % KF滤波的姿态结果
%         
%         Bias_result_KF_Acc = zeros(L,4);
%         Bias_result_KF_Acc(:,1) = IMU(1:L,1);
%         Bias_result_KF_Acc(1,2:4) = Bias';                              %KF滤波的零偏结果
%         
%         XkPk        = zeros(fix(L/KFTime),13);      j = 1;
%         Xk_1Pk_1 = zeros(fix(L/KFTime),13);     j_1 = 1;            %KF 中间量
%         
%         
%         
%     %KF初始化
...
%         KF = InitKF(1,6,3,3);
%         % 初始状态量及方差
%             X0 = [deg2rad([0.5;0.5;1;]);zeros(3,1)]; 
%             P0 = diag(X0.^2);
%             KF.Xk = X0;   KF.Pk = P0;
% 
%         % 状态噪声方差设置
